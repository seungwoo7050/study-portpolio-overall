...
## 8. High-Class Check: 배포, 운영, 설정 관리

전체 로드맵을 마무리하는 시점에서, “로컬에서 잘 돌아가는 코드"를
**실제 서버에 올릴 때 어디서 터지는지**를 정리하는 섹션.  

---

### 8.1 빌드 환경 vs 런타임 환경 분리 (Multi-Stage Docker)

FFmpeg + C++ 애드온 + Node.js 조합은 환경 의존성이 강하다.

패턴:

* **Stage 1 (builder)**

  * 빌드 툴체인(g++, cmake, node-gyp, FFmpeg dev 패키지) 모두 설치
  * `npm ci`, `npm run build`, `node-gyp rebuild` 등 수행
* **Stage 2 (runtime)**

  * 빌드 결과물(`dist/`, `*.node`, 필요한 `*.so`)만 복사
  * 런타임에 불필요한 헤더/컴파일러 제거

이렇게 해야:

* 이미지 용량 감소
* 공격면 축소
* “로컬에서는 되는데 서버에서는 컴파일이…" 같은 문제를 초기부터 차단

---

### 8.2 고가용성(HA)와 상태 저장소

v1.3에서 WebSocket + PostgreSQL + Redis까지 들어가면,
서버를 하나만 쓰는 건 거의 의미가 없다. 

고려할 점:

* **PostgreSQL**

  * 최소: primary + read replica
  * 장애 시 수동 승격, 나중에는 Sentinel / Patroni / Cloud-managed 옵션 검토
* **Redis**

  * 최소: single + persistence
  * 점점 커지면 Sentinel/Cluster 고려
* **WebSocket**

  * 여러 인스턴스를 띄우면, Sticky Session 또는
    Pub/Sub(예: Redis Pub/Sub, NATS 등)으로 진행률 브로드캐스트 동기화 필요

이 프로젝트에서는 설계 문서 수준에서만 언급하고,
구현은 “단일 인스턴스 + 나중에 확장 가능한 구조" 정도로 정리하면 충분하다.

---

### 8.3 설정과 비밀(Secrets) 관리

**코드에 직접 박혀 있으면 안 되는 것들:**
...
* **코드에는 기본값만**(로컬 개발용), 실제 값은 `.env` 또는 환경 변수에서 읽는다.
* Docker / 배포 환경에서는 `.env` 대신, 런타임 환경변수/Secret Store(KMS, Vault 등)를 쓴다.
* C++ 애드온이 경로/설정이 필요하면, JS에서 읽어서 인자로 넘기는 구조로 유지 (애드온 안에서 환경변수 읽는 로직을 최소화).

---

### 8.4 로깅 & 모니터링 최소 기준

Phase 2~3 설계 문서에 이미 Prometheus/Grafana 언급이 있다.  

실제로는 다음 정도는 필요:

1. **구조화 로깅**

* JSON 한 줄 형태로 로그를 남긴다.
* 각 요청/작업에 `requestId` / `operationId`를 붙여서 끝까지 끌고 간다.
...
{
  "level": "info",
  "ts": "2025-11-28T10:20:30Z",
  "requestId": "req-123",
  "operationId": "trim-1732510830",
  "message": "FFmpeg trim started",
  "path": "/api/edit/trim"
}
```
...
* HTTP 레이턴시(p50/p90/p99)
* FFmpeg 처리 시간
* Native Addon 호출 성공/실패 카운트
* 썸네일/메타데이터 요청 횟수
...
### 8.5 배포 전략: 최소 블루-그린 수준 인지

초기에는 “서버 1대 + 수동 재시작"으로 시작해도 된다.
하지만 구조 상, **언젠가는** 아래로 넘어가야 한다:

* **Blue-Green 또는 Rolling Deploy**

  * 새로운 버전을 올릴 때, 기존 트래픽은 유지한 채 새 버전으로 일부 트래픽만 흘려본다.
  * Native Addon, FFmpeg 버전 변경 시 특히 중요.
* **Health Check 기반 로드밸런싱**

  * `/health`에서 native/DB/Redis 상태까지 같이 보고,
    문제가 있는 인스턴스는 로드밸런서에서 제외.

이 프로젝트 문서에서는:

* “Phase 3에서 Docker + Health Check + 간단한 Blue-Green까지 보는 걸 목표로 한다" 정도로만 선언해두면 충분하다.
