...
- C++ 기초가 필요한 경우: [T9.5](./T9.5-cpp-basics.md)부터 시작
...
| 튜토리얼 | 내용 | 예상 시간 | 적용 프로젝트 |
|---------|------|----------|------------|
| [T9.5](./T9.5-cpp-basics.md) | **C++ 기초** - 문법, OOP, 메모리 관리, STL (T10 사전 준비) | 2-4주 | C++ 트랙 전체 |
...
```
T0 → T01 → T02 → T06 → T03 → T04 → T07 → T05 → T08 → T13 → T09
```
**총 예상 시간**: 123-165시간

### 2. 백엔드 개발자 (Node.js)
```
T0 → T01 → T03 → T04 → T05 → T13
```
**총 예상 시간**: 73-100시간

### 3. 게임 서버 개발자 (C++)
```
T0 → T01 → T9.5 → T10 → T11 → T11-2 → T13
```
**총 예상 시간**: 83-115시간 (C++ 기초 있으면 T9.5 생략 가능)

### 4. 멀티미디어 개발자 (C++ + Node.js)
```
T0 → T01 → T02 → T9.5 → T10 → T12 → T13
```
**총 예상 시간**: 73-105시간 (C++ 기초 있으면 T9.5 생략 가능)
...
### [T9.5: C++ 기초 튜토리얼](./T9.5-cpp-basics.md)
- C++ 개요 및 환경 설정
...
| 레벨 | 튜토리얼 | 선행 요구사항 |
|-----|---------|-------------|
| 🟢 **기초** | T0, T01, T02, T06, T9.5 | 프로그래밍 기본 지식 |
| 🟡 **중급** | T03, T04, T07, T13 | 기초 레벨 완료 |
| 🟠 **고급** | T05, T08, T09, T10, T11 | 중급 레벨 완료 (T10은 T9.5 필수) |
