...
#### ✅ Game Server (C++)
- **상태**: gameserver-fundamentals (Lab 1.1-1.4) + netcode-core (1.0-1.3) 완료
- **핵심 기술**: C++17, boost.asio, WebSocket, UDP 넷코드
- **특징**:
  - TCP/WebSocket 기초 → UDP 권위 서버
  - 60 TPS 게임 루프 + 스냅샷/델타 동기화
  - 클라이언트 예측 + 서버 리컨실리에이션
  - 엔티티 보간 + 네트워크 시뮬레이션
...
#### ✅ React Frontend
- **상태**: M1-M6 완료 (프로덕션 완성)
- **핵심 기술**: React 18, TypeScript, Vite, React Query, RHF+zod
- **특징**:
  - Issue Tracker UI (CRUD, 폼 검증, 검색)
  - 통계 대시보드 (React Query + 차트)
  - Elasticsearch 검색 UI + URL 동기화
  - Vitest/Playwright 테스트 + 접근성 (a11y)
...
4. **Game Server**
   ...
   - Q: "스냅샷/델타 동기화는 어떻게 구현했나?"
   - A: 키프레임(전체 상태) + 델타 프레임(변경 사항만), 대역폭 50% 절감
