...
└─────────────────────────────────────────────────────┘
    ...
    │    ├── register_factory(name, fn, singleton=True)
    │    └── resolve(name) → instance
    │
    ├─ [2] Routes (List[Route])
    │    └── Route(path, methods, handler, middleware)
    │
    ├─ [3] Global Middleware (List[Middleware])
    │    └── Middleware = Callable[[Request, Next], Response]
    │
    ├─ [4] HTTP Server (from Milestone 1.1)
    │    └── _handle_connection(socket)
    │
    └─ [5] Request/Response Abstraction
         ├── Request(raw, app) → unified API
         └── Response.from_result() → HTTP bytes
```
...
Client connects
    ↓
1. Parse HTTP request → RawHttpRequest
    ↓
2. Match route → (handler, path_params)
    ↓
3. Create Request(raw, app) → high-level API
    ↓
4. Build middleware pipeline
    [global_middleware] + [route_middleware] + [handler]
    ↓
5. Execute pipeline(request) → result
    ↓
6. Convert result → Response
    ├── dict → JSON response
    ├── str → text response
...
milestone-1.2/
├── framework/
│   ├── app.py              # App class (core orchestration)
│   ├── container.py        # DependencyContainer
│   ├── decorators.py       # Middleware type definition
...
```python
# 1. Return dict → auto JSON
return {"user_id": 42}

# 2. Return str → text response
return "Hello, World!"

# 3. Return Response → full control
...
2. **Middleware Stacking**: Global → Route → Handler
...
We want execution order: `logging → cors → auth → handler`
...
def logging_middleware(request: Request, next_fn: Callable) -> Any:
    ...
    LOGGER.info("→ %s", response.status if isinstance(response, Response) else 200)
    return response
...
- [x] Handler return dict → auto JSON serialization
