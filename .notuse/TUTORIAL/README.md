...
**처음 시작하는 경우:**
1. [T0: 개발 환경 설정](./T0-dev-environment-setup.md)부터 시작하세요 (필수)
2. 본인의 목표에 맞는 [학습 경로](#-추천-학습-순서) 선택
3. 각 튜토리얼을 순서대로 진행

**이미 개발 환경이 준비된 경우:**
- JavaScript/TypeScript 개발: [T01](./T01-js-ts-core.md)부터 시작
- C++ 기초가 필요한 경우: [T10](./T10-cpp-basics.md)부터 시작
...
|---------|------|----------|------------|
| [T0](./T0-dev-environment-setup.md) | **개발 환경 설정** - Homebrew, Node.js, Git, VS Code | 3-5시간 | 전체 |
...
| 튜토리얼 | 내용 | 예상 시간 | 적용 프로젝트 |
|---------|------|----------|------------|
| [T10](./T10-cpp-basics.md) | **C++ 기초** - 문법, OOP, 메모리 관리, STL (T11 사전 준비) | 2-4주 | C++ 트랙 전체 |
| [T11](./T11-cpp-raii-tcp.md) | **Modern C++17 + TCP** - RAII, 소켓, 스레드 | 15-20시간 | game-server lab1.1-1.2 |
| [T12](./T12-asio-websocket-gameloop.md) | **Asio + WebSocket + 게임 루프** - 비동기 IO, 틱 | 18-25시간 | game-server lab1.3-1.4 |
| [T13](./T13-udp-netcode.md) | **UDP 넷코드** - 신뢰성, 예측, 리컨실리에이션 | 25-35시간 | game-server lab1.5-1.6 |
| [T14](./T14-node-api-ffmpeg.md) | **Node-API + FFmpeg C API** - Native addon, 바인딩 | 20-30시간 | video-editor v2.x |

### DevOps / 배포 경로

| 튜토리얼 | 내용 | 예상 시간 | 적용 프로젝트 |
|---------|------|----------|------------|
| [T15](./T15-docker-deployment.md) | **Docker + 배포** - 컨테이너화, 모니터링, CI/CD | 12-18시간 | 전체 프로젝트 |
...
```
T0 → T01 → T02 → T06 → T03 → T04 → T07 → T05 → T08 → T15 → T09
```
**총 예상 시간**: 123-165시간

### 2. 백엔드 개발자 (Node.js)
```
T0 → T01 → T03 → T04 → T05 → T15
```
**총 예상 시간**: 73-100시간

### 3. 게임 서버 개발자 (C++)
```
T0 → T01 → T10 → T11 → T12 → T13 → T15
```
**총 예상 시간**: 83-115시간 (C++ 기초 있으면 T10 생략 가능)

### 4. 멀티미디어 개발자 (C++ + Node.js)
```
T0 → T01 → T02 → T10 → T11 → T14 → T15
```
**총 예상 시간**: 73-105시간 (C++ 기초 있으면 T10 생략 가능)
...
#### game-server
- **lab1.1-1.4 (TCP/WS)**: T01, T10, T11, T12
- **lab1.5-1.6 (UDP 넷코드)**: 위 + T13
...
- **비권장**: C++ (T10-T14)와 다른 트랙 병렬 (인지 부하 높음)
...
### [T0: 개발 환경 설정](./T0-dev-environment-setup.md)
- Homebrew 패키지 관리자 설치 및 사용
- Node.js 환경 구성 (nvm, npm, TypeScript)
- React/Vite 프로젝트 초기 설정
- Node.js/NestJS 프로젝트 환경
- C++ 개발 환경 (컴파일러, CMake)
- Git & GitHub 설정
...
### [T10: C++ 기초 튜토리얼](./T10-cpp-basics.md)
- C++ 개요 및 환경 설정
- 기본 문법 (변수, 타입, 연산자, 제어 구조)
- 함수와 모듈 (헤더 파일, 네임스페이스)
- 객체 지향 프로그래밍 (클래스, 상속)
- 메모리 관리 (포인터, 참조, 동적 메모리)
- STL 기초 (vector, map, 알고리즘)
- 컴파일과 디버깅
- 실습 프로젝트 (콘솔 계산기)

### [T11: Modern C++17 + RAII + TCP 소켓](./T11-cpp-raii-tcp.md)
...
### [T12: Boost.Asio + WebSocket + 게임 루프](./T12-asio-websocket-gameloop.md)
...
### [T13: UDP 넷코드 + 권위 서버](./T13-udp-netcode.md)
...
### [T14: Node-API + FFmpeg C API](./T14-node-api-ffmpeg.md)
...
### [T15: Docker + 배포 + 모니터링](./T15-docker-deployment.md)
...
### game-server 프로젝트
```
lab1.1 (TCP 에코)       → T01, T10, T11
lab1.2 (턴제 전투)      → T10, T11
lab1.3 (WS 채팅)        → T10, T12
lab1.4 (Pong 서버)      → T10, T12
lab1.5 (UDP 신뢰성)     → T10, T13
lab1.6 (넷코드 완성)    → T10, T13 심화
```
...
| 레벨 | 튜토리얼 | 선행 요구사항 |
|-----|---------|-------------|
| 🟢 **기초** | T0, T01, T02, T06, T10 | 프로그래밍 기본 지식 |
| 🟡 **중급** | T03, T04, T07, T15 | 기초 레벨 완료 |
| 🟠 **고급** | T05, T08, T09, T11, T12 | 중급 레벨 완료 (T11은 T10 필수) |
| 🔴 **전문가** | T13, T14 | 고급 레벨 + 관련 도메인 지식 |
